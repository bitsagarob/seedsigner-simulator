#!/usr/bin/env python3
"""The demo: a 2-of-3 MuSig2 spend, signed on the simulated device, on chain.

Bitcoin Core is the coordinator and the second signer. The simulator is the
first. Nothing is faked: the PSBT goes in through the camera as QR codes and the
device's answer comes back off its screen the same way.

  ./musig_demo.py            regtest, the throwaway node
  ./musig_demo.py --shots DIR   where the screenshots go

Screens worth keeping are written as PNGs as they happen.
"""
