...
def image(d):
    ...
    d.move('ArrowDown', 'Skip backup verification')
    d.move('ArrowDown', 'Skip backup verification (second Down)')
...
def cards(d):
    ...
    for i in range(5):
        d.move('ArrowDown', 'Discard seed' if i == 4 else 'Move toward Discard seed')
...
def menus(d):
    d.move('ArrowRight', 'Seeds')
    d.key('Enter', 'ButtonListScreen', 'Load a seed menu')
    for i in range(6):
        d.move('ArrowDown', f'Inspect Load a seed entry {i + 2}')
